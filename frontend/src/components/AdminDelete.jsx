...
export default function AdminDelete() {
  ...
  const [confirm, setConfirm] = useState(null); // id to confirm
              ...
              return (
                <tr key={p._id} style={{ borderBottom: "1px solid var(--bg-tertiary)" }}>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <span style={{ padding: "3px 9px", borderRadius: 20, fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: DIFF[p.difficulty] || "var(--text-muted)", background: `${DIFF[p.difficulty] || "#888"}15` }}>
                      {p.difficulty?.charAt(0).toUpperCase() + p.difficulty?.slice(1)}
                    </span>
                  </td>
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                      {tags.map((t) => <span key={t} style={{ padding: "2px 7px", borderRadius: 5, fontSize: 10, fontFamily: "'JetBrains Mono',monospace", background: "rgba(255,255,255,0.04)", color: "var(--text-muted)", border: "1px solid var(--border)" }}>{t}</span>)}
                    </div>
                  </td>
                  <td style={{ padding: "12px 16px" }}>
